...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get user's team IDs
    const teamIds = memberships.filter(m => m.status === 'ACTIVE').map(m => m.teamId);
    ...
    return NextResponse.json({
      ...
      allGamesInDb: allGames.map(g => ({
        ...g,
        startTime: g.startTime.toISOString(),
        isFuture: g.startTime >= new Date(),
      })),
      gamesForUserTeams: userGames.map(g => ({
        ...g,
        startTime: g.startTime.toISOString(),
      })),
      futureGamesForUser: futureGames.map(g => ({
        ...g,
        startTime: g.startTime.toISOString(),
      })),
    });
